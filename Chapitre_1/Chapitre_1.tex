  ...
  \section{Le paradigme des oscillations des neutrinos}
    ...
    \subsection{Genèse de la théorie}
      ...
      \begin{figure}[htbp]
        ...
        \end{subfigure}
        \caption[Les preuves que les neutrinos changent de saveur, par \gls{sno} et Super-Kamiokande.]{Les deux graphiques ayant prouvé les changement de saveur des neutrinos : le flux total de neutrinos provenant du soleil est conservé mais sa composition de saveur change, par \gls{sno} (gauche). Le nombre de neutrinos muoniques atmosphériques détectés dépend de la ligne de base, par Super-Kamiokander (droite).}
      \end{figure}

      \gls{sno} détectait les neutrinos solaires, par interaction par courant chargé (CC) et par courant neutre (CN), permettant d'avoir accès à la fois au flux de neutrinos électroniques (CN et CC) et au flux de neutrios muoniques et tauiques (CN). Comme le montre la \autoref{fig::SNO_plot}, la somme des trois flux correspond bien au flux total prédit par le modèle solaire standard alors que le flux de neutrinos électroniques est inférieur aux prédictions, montrant qu'une partie du flux change de saveur mais que le flux total est conservé.
    ...
    \subsection{La matrice PMNS}\label{sec::pmns}
      ...
      \subsubsection{Développement limité de la matrice PMNS et terme de violation CP}
        ...
        \end{equation}
        où $A_{atm} = \sin(\theta_{23})\sin(2\theta_{13})\sin\left(\Delta m^2_{31}\frac{L}{4E}\right)$ et $A_{sol} =\cos(\theta_{23})\sin(2\theta_{12})\sin\left(\Delta m^2_{21}\frac{L}{4E}\right)$. Le dernier terme sera sensible à la phase de violation CP.
...
%        \end{align}
  ...
  \section{État de l'art}
    ...
    \subsection{Les connaissances actuelles}\label{sec::current_knowledge}
      ...
      \begin{itemize}
        \item[$\bullet$] Les neutrinos de réacteurs, issus des réactions de fissions au cœur des réacteurs nucléaires, sont des $\anu_e$, avec un spectre en énergie allant de 1 à \SI{10}{\mega\electronvolt}.
