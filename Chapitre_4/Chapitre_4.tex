...
%        \end{minipage}
    ...
    \subsection{Test des CRP du démonstrateur \SSS{} dans une boite cryogénique}
      ...
      \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.7\textwidth]{crp_in_coldbox.png}
...
%                %TODO à faire
  ...
  \section{Efficacité de collection de charge}\label{sec::efficacites}
    ...
    \subsection{Méthodes de simulation}
      ...
      Le logiciel de simulation \gls{ansys}, utilisant la méthode des éléments finis, a été utilisé pour générer les cartes de champs de la zone entre la grille d'extraction et l'anode (voir \autoref{fig::ansys_geom}), pour plusieurs configuration de champs électriques. La supposition est faite que l'efficacité de collection du \gls{lem} n'aura aucune influence sur l'efficacité de collection de l'anode et inversement, ce qui se justifie en regardant la variation du champ à travers le \gls{lem} quand les champ d'extraction et d'induction changent (voir \autoref{fig::champ_dans_lem}). En revanche, le champ d'amplification va avoir une influence sur les deux efficacités. Il a donc été décidé de simuler l'efficacité de collection du \gls{lem} (de l'anode) en fonction du champ d'amplification et du champ d'extraction(d'induction).
            
      Le terme \textit{champ d'extraction} sera employé, comme au chapitre précédent, pour désigner le champ dans le liquide. Par exemple, un \textit{champ d'extraction} de $\SI{2}{\kilo\volt\per\centi\meter}$ correspond environ $\SI{3}{\kilo\volt\per\centi\meter}$ dans le gaz et $\SI{2.5}{\kilo\volt}$ appliqués (voir \eqref{eq::field_liquid}). Cette ambiguïté n'existe pas pour le champ d'induction, qui n'est que dans le gaz avec une constante diélectrique très proche de l'unité, une simple division du potentiel par la distance de \SI{2}{\milli\meter} suffit pour calculer le champ électrique.
    ...
    \subsection{Résultats}
      ...
      \begin{figure}[htpb]
        \begin{subfigure}{\textwidth}
          \centering
          \includegraphics[width=0.48\textwidth]{eff_lem_alone.pdf}
          \includegraphics[width=0.48\textwidth]{eff_lem_alone_1D.pdf}
        ...
        \begin{subfigure}{\textwidth}
          \centering
          \includegraphics[width=0.48\textwidth]{eff_lem_guschin.pdf}
          \includegraphics[width=0.48\textwidth]{eff_lem_guschin_1D.pdf}
        ...
        \begin{subfigure}{\textwidth}
          \centering
          \includegraphics[width=0.48\textwidth]{eff_anode.pdf}
      ...
      \end{figure}
      La \autoref{fig::eff_lem_alone} montre l'efficacité de collection du \gls{lem} , la \autoref{fig::eff_lem_guschin} montre la convolution de cette efficacité à l'efficacité d'extraction décrite en \autoref{sec::extraction}. On constate qu'avec cette convolution, on retrouve bien un maximum local entre \SI{2}{\kilo\volt\per\centi\meter} et \SI{2.5}{\kilo\volt\per\centi\meter}, confirmant notre hypothèse initiale. On remarque de plus que l'efficacité n'atteint jamais 1. Enfin, un champ d'amplification plus grand permettra une meilleure collection de la charge à champ d'extraction fixé. L'efficacité de collection de l'anode, montrée sur la \autoref{fig::eff_anode}, quant à elle croît avec le champ d'induction mais décroît avec le champ d'amplification. Aux champs d'amplifications requis par \gls{dune}, elle ne sera jamais de 1 avec une induction de \SI{5}{\kilo\volt\per\centi\meter}. La simulation n'a pas été effectué au delà de cette valeur pour des raisons de temps, mais il serait intéressant de vérifier le comportement à plus haute induction.
