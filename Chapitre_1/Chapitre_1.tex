\chapter{Contexte théorique et expérimental}
    ...
    \section{De la nécessité théorique à la place du neutrino dans la modèle standard}
	    ...
	    kb
		    \begin{figure}
		    	\centering
		    	\includegraphics[width=0.8\textwidth]{Standard_Model_Of_Particle_Physics--Most_Complete_Diagram.png}\\
		    	\tiny{\textcopyright L. Boyle, Creative Commons}
		    	\caption[Modèle standard de la physique des particules.]{\label{fig::SM}Le modèle standard de la physique des particules comprend 3 types de particules : un boson de spin 0, le boson de Higgs, responsable de la masse après brisure de la symétrie du vide (brisure qui mélange les interactions faible et électromagnétique). 12 bosons de spin 1, médiateurs des interactions faibles : 1 photon $\gamma$ pour l'électromagnétisme qui interagit avec les particules possédant un charge électrique $Q$, 8 gluons $g$ pour l'interaction forte, responsable de la cohésion du noyau atomique, qui interagit avec les particules possédant une charge de couleur rouge verte ou bleu, 2 bosons chargés $W^{\pm}$ et 1 boson neutre $Z^0$ pour l'interaction faible qui interagit avec les doublets de fermions de chiralité gauche. Et enfin 12 fermions de spin $1/2$, réparties en 3 générations (les trois colonnes de fermions sur la figure). La première génération constitue la matière stable qui nous entoure, les particules dans les deux autres familles sont lourdes et se désintègrent (sauf les trois neutrinos). Les deux lignes du haut du tableau de fermions sont les quarks, sensibles aux trois interactions, et qui ne peuvent exister seuls mais doivent former des hadrons (comme le proton et le neutron, mais aussi des particules instables comme les mésons). Les deux lignes du bas sont les leptons. Les leptons chargés $e$, $\mu$ et $\tau$ sont sensibles à l'interaction électromagnétique et faible, les neutrinos ne sont sensibles qu'à l'interaction faible. Tous ces fermions possèdent une antiparticules, identique en terme de masse mais de charge électrique, d'isospin faible et de charge de couleur opposée. Le modèle standard n'inclue pas la gravitation.}
		    \end{figure}
