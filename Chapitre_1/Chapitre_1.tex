...
%    }
  ...
  \section{Le paradigme des oscillations des neutrinos}
    ...
    \end{activitybox}
      ...
      \gls{sno} détectait les neutrinos solaires, par interaction par courant chargé (CC) et par courant neutre (NC), permettant d'avoir accès à la fois au flux de neutrinos électroniques (NC et CC) et au flux de neutrinos muoniques et tauiques (NC). Comme le montre la \autoref{fig::SNO_plot}, la somme des trois flux correspond bien au flux total prédit par le modèle solaire standard alors que le flux de neutrinos électroniques est inférieur aux prédictions, montrant qu'une partie du flux change de saveur mais que le flux total est conservé.
